...
class Farmer(s: Simulation, obs: Observator, _landAdmin: LandAdministrator, _age: Int, _children: List[Child]) extends SimO(s) {
    ...
    var municipality : List[Farmer] = List()
    ...
    type T <: LandOverlay
    def getOracleStartegy(budget: Double, landResources: List[CadastralParcel]) : List[T] = ???
