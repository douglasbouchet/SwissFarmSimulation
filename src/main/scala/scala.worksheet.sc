...
/** @note
  *   This class is in charge of generating data for the simulation engine The
  *   area are in ha It generates the following:
  ...
  * - The agglomerations: "x = n inhabitants" We create villages(x < 2k), town
  * (2k < x < 5k), small city (5k < x < 20k), medium city (20k < x < 50k), big
  * city (50k > x) TODO comment distancer les villes ? inclure une dimension
  * spatiale ?
  * - People based on these cities
  * - Other entities than farms involved in food supply chain For the moment we
  * do not have any spatial locality, so no ways to known which parcels are
  * neighbors Each generated type of data is stored inside one excel file
  */
...
val f = new File(
  "/Users/douglasbouchet/Desktop/SwissFarmSimulation/src/main/data/statistical_data/canton_stats.xlsx"
)
val workbook = WorkbookFactory.create(f)
val sheet = workbook.getSheetAt(0)

/** this will be used to assign a number of parcels to each farm They should be
  * store as variable of the class generator cause used in some of its methods
  */
var nbFarmPerCanton, nbFarmMore30ha, nbFarmMore10Less30,
    nbFarmLess10: List[(String, Int)] = List()
...
for (i <- 1 to 27) {
  nbFarmPerCanton = (
    ...
    math.round(sheet.getRow(i).getCell(1).toString().toDouble).toInt
  ) :: nbFarmPerCanton
  nbFarmMore30ha = (
    sheet.getRow(i).getCell(0).toString(),
    math.round(sheet.getRow(i).getCell(2).toString().toDouble).toInt
  ) :: nbFarmMore30ha
  nbFarmMore10Less30 = (
    sheet.getRow(i).getCell(0).toString(),
    math.round(sheet.getRow(i).getCell(3).toString().toDouble).toInt
  ) :: nbFarmMore10Less30
  nbFarmLess10 = (
    sheet.getRow(i).getCell(0).toString(),
    math.round(sheet.getRow(i).getCell(4).toString().toDouble).toInt
  ) :: nbFarmLess10

  /** This numbers seems a bit low, check afterwards total use for agricultural
    * purpose
    */
  totalCropsArea = (
    sheet.getRow(i).getCell(0).toString(),
    math.round(sheet.getRow(i).getCell(6).toString().toDouble).toInt
  ) :: totalCropsArea
  totalWheatCropsArea = (
    sheet.getRow(i).getCell(0).toString(),
    math.round(sheet.getRow(i).getCell(10).toString().toDouble).toInt
  ) :: totalWheatCropsArea
  totalSurface = (
    sheet.getRow(i).getCell(0).toString(),
    math.round(sheet.getRow(i).getCell(14).toString().toDouble).toInt * 100
  ) :: totalSurface
  population = (
    sheet.getRow(i).getCell(0).toString(),
    math.round(sheet.getRow(i).getCell(15).toString().toDouble).toInt
  ) :: population
}
...
/** Next we construct the parcels We differentiate 2 types of parcels, the
  * agricultural ones, the other agricultural parcels range from 2 to 10 ha,
  * following gaussian distribution of mean 5, var TBD (rm external values)
  * (this was a quick guess, try to find more precise infos) other ranges from
  * 0.03 ha to 2 ha, gaussian distrib of mean 0.06, var TBD
  */

/** Generate random areas until a total area is reached, following a gaussian
  * distribution If generated area outside [min,max], generates a new one
  * @param min:
  *   Double, the minimum value
  * @param max:
  *   Double, the maximum value
  * @param mean:
  *   Double, mean of the gaussian
  * @param variance:
  *   Double, variance of the gaussian
  * @param until:
  *   Double, the total area we want to achieve
  */
def generateRdmArea(
    min: Double,
    max: Double,
  ...
  while (remainingArea > 0) {
    sample = BigDecimal(gaussianDist.sample())
...
/** TODO add some statistics about each commune in switzerland generate parcels
  * for each of this communes based on these statististics (to give id to
  * parcels)
  */
def generateParcels(
    canton: String
): (List[CadastralParcel], List[CadastralParcel]) = {
  ...
  generateRdmArea(2, 10, 5, 2.4, cropAreas).foreach(area => {
    agriculturalParcels ::= new CadastralParcel(
      ("TBD", 0),
      new Owner,
      List(),
      area
  ...
  generateRdmArea(0.03, 2, 0.06, 2.4, cropAreas).foreach(area => {
    otherParcels ::= new CadastralParcel(("TBD", 0), new Owner, List(), area)
  })

  (agriculturalParcels, otherParcels)
}

/** Assign in a round robin manner an amount of parcels to a small, medium, big
  * farm. Repeat until there is no parcels remaining For each farm: Choose a
  * number of ha as following: small farm: from 2 to 9 (Uniform) medium farm:
  * from 10 to 29 (Uniform) big farm: from 30 to 60 (Uniform) Assign parcels
  * until area reach the number of ha
  */
...
/** next step is to create some land overlays TODO see how to do
  */

/** generate people for each canton */
def generatePeople(canton: String, sim: Simulation): List[Person] = {
  (for (i <- 1 to population.filter(_._1 == canton).head._2)
    yield new Person(sim, false)).toList
}
...
/** Next we generate the road network Idea: get data for generating national
  * roads in switzerland Next generate as an Ldiagram growing from the highway
  * the others roads
  */

val networkClass = new RoadNetwork

val network: scalax.collection.mutable.Graph[Intersection, EdgeRoad] =
  networkClass.roadNetwork
...
/** Should generate the nodes based on some stats about canton/communes The idea
  * is to get some clusters representing the cities. and some intermediates
  * nodes between the cities to make junction between them
  */
def generateNodes(n: Int): List[Intersection] =
  (for (i <- 0 to n - 1) yield new Intersection(i.toString())).toList

/** add edges in a random way. If graph not connected until a certain amount of
  * iteration, connect the remaining isolated nodes
  */
def generateEdges(roadNetworkInstance: RoadNetwork) = {
  var road = null
  val networkGraph: scalax.collection.mutable.Graph[Intersection, EdgeRoad] =
    roadNetworkInstance.roadNetwork
  val nNodes = networkGraph.nodes.length
  var nRoads: Int = 0
  while (!networkGraph.isConnected && nRoads < 100) {
    val fromNode: Intersection =
      networkGraph.nodes.toList(scala.util.Random.nextInt(nNodes))
    val toNode: Intersection =
      networkGraph.nodes.toList(scala.util.Random.nextInt(nNodes))
    //check if node !=
...
/** we make edges on the graph until it is connected, add random edge Then we
  * create an empty Set. Create tuple in the form (Node, Int), the node and its
  * key Apply the algo
  */
...
/** We know the number of farms per canton. We assign them a random number of
  * parcels of agricultural purpose
  */
